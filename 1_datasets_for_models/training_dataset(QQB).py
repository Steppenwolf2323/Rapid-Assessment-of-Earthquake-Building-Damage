...
# ----------------------------------------------------------------------
# Quick smoke-test  (only runs when the file is executed directly)
# ----------------------------------------------------------------------
if __name__ == "__main__":
    ...
    #   Thesis/
    #     1_datasets_for_models/    <- this file lives here
    #     data_preprocessing/
    #       qqb_dataset/
    #         qqb_train.csv         <- produced by qqb_preprocessing.ipynb
    def find_repo_root(start: Path) -> Path:
        """Walk up the directory tree until we find the .git folder."""
        for parent in [start, *start.parents]:
            if (parent / ".git").exists():
                return parent
        raise FileNotFoundError("Could not find the git repo root.")

    _REPO_ROOT = find_repo_root(Path(__file__).resolve())
    csv_path = str(_REPO_ROOT / "0_data_preprocessing" / "qqb_dataset" / "qqb_train.csv")

    # --- Model A: plain dataset ---
    dataset_A = QQBDataset(csv_file=csv_path, normalize=True, augment=True)
    img, lbl = dataset_A[0]
    print(f"[Model A] image shape: {img.shape}  |  label: {lbl.item()}")
    print(f"          pixel range: [{img.min():.3f}, {img.max():.3f}]")

    # --- Model B hook: pass a custom transform with masking ---
    # (replace the lambda below with your actual masking logic)
    from torchvision import transforms as T
    model_b_transform = T.Compose([
        T.Resize((224, 224)),
        T.ToTensor(),
        T.Normalize(mean=IMAGENET_MEAN, std=IMAGENET_STD),
    ])
    dataset_B = QQBDataset(csv_file=csv_path, transform=model_b_transform)
    img_b, lbl_b = dataset_B[0]
    print(f"[Model B] image shape: {img_b.shape}  |  label: {lbl_b.item()}")

    # --- Model C: no metadata yet (approximation mode) ---
    def shadow_pre_transform(pil_img: Image.Image) -> Image.Image:
        # Placeholder — replace with actual physics-based shadow logic
        return pil_img

    dataset_C = QQBDataset(csv_file=csv_path, normalize=True, pre_transform=shadow_pre_transform)
    result_c = dataset_C[0]           # returns (image, label) — no metadata columns in CSV
    img_c, lbl_c = result_c[0], result_c[1]
    print(f"[Model C – no metadata]   image: {img_c.shape}  |  label: {lbl_c.item()}")
    print(f"  has_metadata: {dataset_C.has_metadata}")

    # --- Model C: real metadata available ---
    # Simulates a CSV that already has sun metadata columns populated.
    # When you obtain the real metadata, just add the columns to your CSV
    # and the dataset will automatically return the (image, label, metadata) triple.
    import tempfile, os
    df_meta = pd.read_csv(csv_path)
    df_meta["sun_azimuth"]   = 137.5   # degrees clockwise from north
    df_meta["sun_elevation"] = 42.3    # degrees above horizon
    with tempfile.NamedTemporaryFile(mode="w", suffix=".csv", delete=False) as tmp:
        df_meta.to_csv(tmp.name, index=False)
        tmp_path = tmp.name

    dataset_C_meta = QQBDataset(csv_file=tmp_path, normalize=True, pre_transform=shadow_pre_transform)
    img_cm, lbl_cm, meta = dataset_C_meta[0]   # unpacks the triple
    print(f"[Model C – with metadata] image: {img_cm.shape}  |  label: {lbl_cm.item()}")
