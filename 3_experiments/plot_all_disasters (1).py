"""
plot_all_disasters.py — Thesis figures using full xBD test set (324 samples)
Includes mexico-earthquake, palu-tsunami, sunda-tsunami.

Place in: 3_experiments/
Reads:    evaluation_xbd.json from each model folder
Saves:    3_experiments/figures_all_disasters/

Usage:
    cd 3_experiments
    python plot_all_disasters.py
"""
...
FIGURES_DIR = BASE / "3_experiments" / "figures_all_disasters"
...
def load_history(path: Path) -> dict:
    ...
    Handles two history formats:
        Models A/B: {"stage1": [...], "stage2": [...]}  (two-stage training)
        Model C:    [{...}, {...}, ...]                 (single-stage training)
    """
    with open(path) as f:
        h = json.load(f)

    # Flatten into one list of entries regardless of format
...
def plot_summary_table(histories, evaluations):
    ...
    ax.set_title("Results: QQB validation vs xBD test (all disasters)", fontsize=12, pad=20)
    fig.tight_layout()
...
def plot_per_disaster(evaluations):
    ...
    for metric, ax in zip(["f1", "recall"], axes):
        for i, (model_id, ev) in enumerate(evaluations.items()):
            vals = [ev["per_disaster"].get(d, {}).get(metric, 0) for d in disasters]
            bars = ax.bar(x + (i - 1) * width, vals, width,
                          label=LABELS[model_id], color=COLORS[model_id], alpha=0.85)
...
def plot_roc_curves(evaluations):
    ...
    ax.set_title("ROC curves — xBD test set (all disasters)")
    ax.legend(fontsize=9)
    fig.tight_layout()
